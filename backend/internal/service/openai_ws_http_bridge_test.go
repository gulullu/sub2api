...
func TestPrepareOpenAIWSHTTPBridgeBodyPreservesResponsesLiteRequiredFallback(t *testing.T) {
	...
	body, err := prepareOpenAIWSHTTPBridgeBody(nil, normalized)
...
func TestProxyOpenAIWSHTTPBridgeTurnClientDisconnectStillDrainsCompleted(t *testing.T) {
	gin.SetMode(gin.TestMode)
	body := strings.Join([]string{
		`data: {"type":"response.created","response":{"id":"resp_disconnected","status":"in_progress"}}`,
		``,
		`data: {"type":"response.output_text.delta","response_id":"resp_disconnected","delta":"hello"}`,
		``,
		`data: {"type":"response.completed","response":{"id":"resp_disconnected","status":"completed","usage":{"input_tokens":7,"output_tokens":3}}}`,
		``,
	}, "\n")
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
...
func TestOpenAIWSHTTPBridgeAcceptsFirstFrameAboveLegacy16MiB(t *testing.T) {
	...
	payload := []byte(strings.Repeat(" ", 1024) + `{"type":"response.create","generate":true,"model":"gpt-5","stream":true,"reasoning":{"effort":"xhigh"},"input":"` + strings.Repeat("x", 17*1024*1024) + `"}`)
	...
	case outcome := <-turnOutcomeCh:
		...
		require.True(t, outcome.result.UpstreamCompleted)
		require.False(t, outcome.result.ClientDisconnect)
		require.NotNil(t, outcome.result.ReasoningEffort)
		require.Equal(t, "high", *outcome.result.ReasoningEffort)
		require.NotNil(t, outcome.result.RequestedReasoningEffort)
		require.Equal(t, "xhigh", *outcome.result.RequestedReasoningEffort)
	default:
