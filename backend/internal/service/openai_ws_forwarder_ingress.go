...
func (s *OpenAIGatewayService) ProxyResponsesWebSocketFromClient(
	...
	parseClientPayload := func(turn int, raw []byte) (openAIWSClientPayload, error) {
		...
		// Keep the pre-scoped payload for session/hash and audit identity
		// bookkeeping. The outbound payload may be rewritten per account below.
		...
		if sparkContextChanged {
			normalized = normalizedSparkPayload
		}
	...
	if forceHTTPBridge || s.shouldBridgeOpenAIWSHTTP(account, firstPayload.payloadBytes, firstPayload.previousResponseID) {
		...
		for turn := 1; ; turn++ {
			...
			if result != nil {
				// parseClientPayload captures this before group policy rewrites the
				// outbound effort. The HTTP bridge otherwise only sees the rewritten
				// body and would collapse requested and effective attribution.
