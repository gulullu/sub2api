...
func TestPassthroughLifecycle_ResponsesLiteSparkMappedModelPersistsWhenFollowupOmitsModel(t *testing.T) {
	...
	require.True(t, gjson.GetBytes(firstWrite, `tools.#(type=="namespace")`).Exists(), string(firstWrite))
	require.True(t, gjson.GetBytes(firstWrite, `tools.#(type=="tool_search")`).Exists(), string(firstWrite))
	require.False(t, gjson.GetBytes(firstWrite, `input.#(type=="additional_tools")`).Exists(), string(firstWrite))
	require.Equal(t, "required", gjson.GetBytes(firstWrite, "tool_choice").String())
	require.Equal(t, gjson.True, gjson.GetBytes(firstWrite, "parallel_tool_calls").Type, string(firstWrite))
	...
	require.True(t, gjson.GetBytes(secondWrite, `tools.#(type=="namespace")`).Exists(), string(secondWrite))
	require.True(t, gjson.GetBytes(secondWrite, `tools.#(type=="tool_search")`).Exists(), string(secondWrite))
	require.False(t, gjson.GetBytes(secondWrite, `input.#(type=="additional_tools")`).Exists(), string(secondWrite))
	require.Equal(t, "required", gjson.GetBytes(secondWrite, "tool_choice").String())
	require.Equal(t, gjson.True, gjson.GetBytes(secondWrite, "parallel_tool_calls").Type, string(secondWrite))
