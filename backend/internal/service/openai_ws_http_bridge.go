...
func (s *OpenAIGatewayService) proxyOpenAIWSHTTPBridgeTurn(
	...
	buildUpstreamRequest := func(requestBody []byte) (*http.Request, error) {
		...
		// The Lite marker originates from the client WS frame, but the final
		// request body may carry a mapped/fallback model. Guard each freshly
		// built attempt so a known non-Lite account model never receives the
		// private Lite header, including rejected-field retries.
		guardOpenAIResponsesLiteHTTPHeader(upstreamReq, account, requestBody)
