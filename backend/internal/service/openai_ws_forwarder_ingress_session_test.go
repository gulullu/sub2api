...
func TestOpenAIGatewayService_ProxyResponsesWebSocketFromClient_ResponsesLiteSparkMappedModelAndFollowupCreateCanOmitModel(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	captureConn := &openAIWSCaptureConn{
		events: [][]byte{
			[]byte(`{"type":"response.completed","response":{"id":"resp_spark_lite_1","model":"gpt-5.3-codex-spark","usage":{"input_tokens":1,"output_tokens":1}}}`),
			[]byte(`{"type":"response.completed","response":{"id":"resp_spark_lite_2","model":"gpt-5.3-codex-spark","usage":{"input_tokens":1,"output_tokens":1}}}`),
		},
	}
	captureDialer := &openAIWSCaptureDialer{conn: captureConn}
	pool := newOpenAIWSConnPool(cfg)
	...
	account := &Account{
		ID:          116,
		Name:        "openai-ingress-spark-lite",
		Platform:    PlatformOpenAI,
	...
	require.Equal(t, "resp_spark_lite_1", gjson.GetBytes(readMessage(), "response.id").String())
	...
	require.Equal(t, "resp_spark_lite_2", gjson.GetBytes(readMessage(), "response.id").String())
	_ = clientConn.Close(coderws.StatusNormalClosure, "done")
	...
	case serverErr := <-serverErrCh:
		require.NoError(t, serverErr)
	case <-time.After(5 * time.Second):
		t.Fatal("waiting for Spark ingress websocket shutdown timed out")
	}
	...
	for _, write := range captureConn.writes {
		...
		require.True(t, gjson.Get(forwarded, `tools.#(type=="namespace")`).Exists(), forwarded)
		require.True(t, gjson.Get(forwarded, `tools.#(type=="tool_search")`).Exists(), forwarded)
		require.False(t, gjson.Get(forwarded, `input.#(type=="additional_tools")`).Exists(), forwarded)
		require.Equal(t, "required", gjson.Get(forwarded, "tool_choice").String())
		require.Equal(t, gjson.True, gjson.Get(forwarded, "parallel_tool_calls").Type, forwarded)
	}
	require.Equal(t, "resp_spark_lite_1", gjson.Get(requestToJSONString(captureConn.writes[1]), "previous_response_id").String())
}
...
func TestOpenAIGatewayService_ProxyResponsesWebSocketFromClient_CodexImageBridgeRespectsResponsesLite(t *testing.T) {
	...
	writeCtx, cancelWrite := context.WithTimeout(context.Background(), 3*time.Second)
	// Keep the RelayBases model used by the image-bridge assertions while
	// retaining the upstream regression coverage for boolean normalization and
	// lossless large-number passthrough.
	err = clientConn.Write(writeCtx, coderws.MessageText, []byte(`{"type":"response.create","model":"gpt-5.6-terra","stream":false,"input":"draw a cat","parallel_tool_calls":true,"sequence":900719925474099312345}`))
	cancelWrite()
	require.NoError(t, err)
