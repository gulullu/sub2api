...
func TestConfidenceJSONScannerUsesDeepSeekChatContract(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		require.Contains(t, system["content"], fixedChinesePoliticalSafetyPolicy)
		require.Equal(t, 1, strings.Count(system["content"].(string), fixedChinesePoliticalSafetyPolicy))
