...
func TestConfidenceJSONScannerUsesDeepSeekChatContract(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		require.Contains(t, systemContent, "custom policy")
		require.Contains(t, systemContent, fixedMultilingualSemanticPolicy)
		require.Equal(t, 1, strings.Count(systemContent, fixedMultilingualSemanticPolicy))
		require.Contains(t, systemContent, fixedConfidenceReasonPolicy)
		require.Equal(t, 1, strings.Count(systemContent, fixedConfidenceReasonPolicy))
		require.Contains(t, systemContent, fixedChinesePoliticalSafetyPolicy)
		require.Equal(t, 1, strings.Count(systemContent, fixedChinesePoliticalSafetyPolicy))
...
func TestConfidenceJSONMultilingualPolicyAppliesToEveryFailoverNode(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		require.Contains(t, payload.Messages[0].Content, fixedChinesePoliticalSafetyPolicy)
		require.Equal(t, 1, strings.Count(payload.Messages[0].Content, fixedChinesePoliticalSafetyPolicy))
		calls.Add(1)
