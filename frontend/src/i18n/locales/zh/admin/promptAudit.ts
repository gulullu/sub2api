export default {
  promptAudit: {
    ...
    groups: {
      ...
      profileSearch: '用户名称', profileSearchPlaceholder: '用户名 / 邮箱 / 用户 ID', profileDays: '天数', profileRefresh: '刷新画像', profileColumns: '列设置', profileFixedColumns: '用户列始终显示。', selectPage: '选择本页', clearPage: '清空本页', loadingProfiles: '正在加载用户画像…', profileLoadError: '无法加载用户画像。', noProfiles: '本分组没有匹配的用户。', noProfile: '暂无画像', profileSummary: '{total} 个用户 · 第 {page} / {pages} 页', previous: '上一页', next: '下一页', excludedCount: '已排除 {count} 个用户的本地审核', excludedUsersTitle: '已排除用户', removeExcludedUser: '取消排除 {identity}', excludedPreviewMore: '另有 {count} 个用户未在摘要中展示。', noEmail: '无邮箱', user: '用户', userEmail: '邮箱', userStatus: '状态', userRisk: '内容风险', userCyber: 'CYB 命中', userSamples: '样本数', userCoverage: '审计覆盖', userRecent: '最近活动', userExcluded: '本地审核', deleted: '已删除', unknownStatus: '未知状态', excluded: '已排除', included: '未排除', saveHint: '修改会保留在草稿中，点击页面底部保存后生效。',
    },
    probeGovernance: {
      title: '探针治理',
      description: '为已纳入审计范围的分组单独配置探针识别、最小检测间隔和真实监控豁免。',
      summary: '{count} 个审计分组',
      group: '审计分组', groupFilter: '审计分组', allGroups: '全部分组', searchGroups: '搜索分组名称 / ID',
      status: '治理状态', statusFilter: '治理状态', enabled: '已启用', disabled: '未启用', enableAction: '启用', disableAction: '停用',
      interval: '最小间隔', firstReal: '首次真实检测', localResponses24h: '本地应答', skippedAudits24h: '跳过审核', skippedUpstream24h: '跳过上游', lastProbe: '最近探针',
      last24Hours: '近 24 小时', minutes: '{count} 分钟', seconds: '{count} 秒', columns: '列设置', empty: '当前没有已纳入审计范围的分组。',
      defaults: {
        healthyResponse: '服务正常。为避免重复检测，探针最小间隔为 5 分钟。',
        violationResponse: '服务正常，但无法协助该请求。为避免重复检测，探针最小间隔为 5 分钟。',
        unknownResponse: '网关在线，上游状态正在刷新。探针最小间隔为 5 分钟。',
      },
      policy: {
        title: '编辑探针治理策略 · {name}', enabled: '启用探针治理', requestPolicy: '请求策略', requestPolicyHint: '同一类探针在窗口内只执行一次必要检测，其余请求由网关直接应答。',
        interval: '探针最小间隔（秒）', intervalHint: '允许 60–86,400 秒，默认 300 秒。', intervalError: '请输入 60–86,400 的整数秒数。',
        healthScope: '健康窗口范围', healthScopeGroupModelProtocol: '分组 + 模型 + 协议', healthScopeHint: '跨用户和 API Key 共享，避免更换身份或模型别名绕过。',
        allowFirstReal: '首次健康探针允许真实检测', allowFirstRealHint: '每个健康窗口最多一个请求访问真实上游。',
        skipRepeatAudit: '重复探针跳过审核', skipRepeatAuditHint: '同一策略版本下，同一探针族只审核一次。',
        skipRepeatUpstream: '重复探针跳过上游', skipRepeatUpstreamHint: '不选择账号、不占并发、不计费。',
        localResponses: '本地成功响应', localResponsesHint: '返回与实际协议匹配的 HTTP 200 响应；不会产生用量或影响账号状态。',
        healthyResponse: '健康探针响应', violationResponse: '已确认违规探针响应', unknownResponse: '健康状态刷新中响应', responseError: '响应文案必须包含 1–1,000 个字符。',
      },
      verdicts: { healthy: '健康探针', confirmed_violation: '已确认违规', unknown: '待确认' },
      classifications: { known_health: '已知健康探针', candidate: '候选探针', healthy: '健康探针', confirmed_violation: '已确认违规', unknown: '待确认', cleared: '待重新评估' },
      responseKinds: { upstream: '上游响应', healthy: '本地健康响应', violation: '本地拒答', unknown: '本地状态响应' },
      ...
      handling: {
        forwarded_real_probe: '真实健康检测', audit_then_forward: '首次审核并请求上游', local_healthy: '本地 200 健康应答', local_violation: '本地 200 拒答', local_unknown: '本地 200 状态刷新', audit_blocked: '审核阻止后本地应答', audit_unavailable: '审核暂不可用，本地应答', pending_coalesced: '等待中的同类探针已合并',
      },
      events: {
        action: '事件', title: '探针事件 · {name}', timeRange: '时间范围', last24Hours: '最近 24 小时', last7Days: '最近 7 天', last30Days: '最近 30 天', allTime: '全部时间', customRange: '自定义时间', startAt: '开始时间', endAt: '结束时间',
        verdict: '探针判定', userFilter: '用户（邮箱 / ID）', apiKeyFilter: 'API Key（名称 / ID）', allUsers: '全部用户', allApiKeys: '全部 API Key', model: '模型', protocol: '协议',
        family: '探针族', firstLast: '首次 / 最近', user: '用户', apiKey: 'API Key', modelProtocol: '模型 / 协议', repeats: '重复次数', handling: '处理方式', stream: '流式', nonStream: '非流式',
      ...
      exemptions: {
        action: '豁免', title: '真实监控豁免 · {name}', addTitle: '添加豁免', description: '豁免对象仍留探针事件，只允许安全健康探针执行真实检测；已确认违规永不绕过。',
        targetType: '对象类型', user: '用户', apiKey: 'API Key', selectUser: '搜索用户邮箱 / ID', selectApiKey: '搜索 API Key 名称 / ID', reason: '原因', reasonPlaceholder: '例如：内部真实健康监控', expiresAt: '有效期', expiresAtHint: '留空表示长期有效', expiresAtError: '有效期必须晚于当前时间。', add: '添加豁免',
